	...
	getChatList(userId, userSocketId){
		...
		try {
			return Promise.all([
				...
				// this.db.query(`SELECT * FROM user LEFT JOIN message WHERE from_user_id =? `, [userId])
				this.db.query(`SELECT user.id, username, online, socketid FROM message INNER JOIN user WHERE from_user_id=? group by to_user_id`,[userId])
			]).then( (response) => {
				return {
	...
	async insertMessages(params){
		try {
			return await this.db.query(
				"INSERT INTO message (`from_user_id`,`to_user_id`,`message`, `date`) values (?,?,?)",
